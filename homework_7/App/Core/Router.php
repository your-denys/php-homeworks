...
class Router
{
    ...
    private function validate(): void
    {
        ...
        if (!isset($this->config[$routeKey])) {
            ...
            $this->controller = $this->zone === 'admin' ? $this->config[1] : $this->config[0]; // Должен быть 'Dashboard'
            $this->method = $this->zone === 'admin' ? $this->config[2] : $this->config[1]; // Должен быть 'Dashboard'
        }
    }
